...
void main() {
  group('Backend Authority & Client Untrusted Score Verification', () {
    test('Reaction: Authoritative scorer recalculates from raw timestamps, overriding client claim', () {
      ...
      expect(authoritativeResult.validationStatus, equals(ValidationStatus.valid));
      expect(authoritativeResult.formattedMetric, equals('250 ms'));
    });

    test('Reaction: False start (<100ms) is rejected by server authority regardless of client claim', () {
      final def = ChallengeRegistry.getDefinition(ChallengeType.reaction);
      const config = ReactionConfig(seed: 1337, waitDelayMs: 2500);
      ...
      final authoritativeResult = def.scorer.score(config, falseStartEvidence);

      expect(authoritativeResult.normalizedScore, equals(0));
      expect(authoritativeResult.validationStatus, equals(ValidationStatus.earlyFault));
      expect(authoritativeResult.formattedMetric, equals('FAULT'));
    });

    test('Precision: Out of bounds tap (d > 0.12) is scored as 0 (MISS) by server authority', () {
      ...
      final authoritativeResult = def.scorer.score(config, outOfBoundsEvidence);

      expect(authoritativeResult.normalizedScore, equals(0));
      expect(authoritativeResult.formattedMetric, equals('MISS'));
    });

    test('MatchSession terminates early as soon as target wins (4) are reached', () {
      const scheduled = [
