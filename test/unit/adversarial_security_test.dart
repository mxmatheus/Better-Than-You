...
void main() {
  group('Adversarial Anti-Cheat & Score Authority Tests', () {
    test('Reaction: Server ignores client claimed 0 score when evidence is excellent (200ms)', () {
    ...
    test('Reaction: Server rejects client claimed 1000 score when evidence is poor (550ms)', () {
      final def = ChallengeRegistry.getDefinition(ChallengeType.reaction);
      final config = def.generator.generate(1337);
    ...
    test('Reaction: False start (<100ms) produces strictly 0 score regardless of claim', () {
      final def = ChallengeRegistry.getDefinition(ChallengeType.reaction);
      final config = def.generator.generate(1337);
    ...
    test('Reaction: Boundary timing at exactly 100ms yields maximum 1000 score', () {
      final def = ChallengeRegistry.getDefinition(ChallengeType.reaction);
      final config = def.generator.generate(1337);
    ...
    test('Precision: Out-of-bounds tap (x > 1.0, y < 0.0) yields strictly 0 score (MISS) and FLAGGED', () {
    ...
    test('Memory: Incomplete / wrong sequence receives only accuracy points and 0 speed bonus', () {
  ...
  group('Replay Attacks & Idempotency Verification', () {
    test('Elo calculation is strictly deterministic and idempotent across repeated invocations', () {
    ...
    test('Daily Challenge repository prevents duplicate completions after round 10', () async {
